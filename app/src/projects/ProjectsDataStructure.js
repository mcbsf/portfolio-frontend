
import Logo from '../assets/logo192.png';
export const ProjectsDataStructure = [
    {
        title: 'Portfolio',
        description: 'Project to create this web app.',
        githubUrl: 'https://www.github.com/mcbsf/portfolio-frontend',
    },
    {
        title: 'Redis Like',
        description: 'Project to simulate Redis, with all basic commands available to be used via sheel or importing the Redisclass and using it',
        githubUrl: 'https://www.github.com/mcbsf/redis_like',
    },
    {
        title: 'Legislators Analyser',
        description: 'Project to extract analysis about the relation between legislators, bills and votes',
        githubUrl: 'https://github.com/mcbsf/legislation-bills-analyser',
    },
    {
        title: 'Requirements Engineering',
        description: 'An academic project in which we choose a proccess to map requirements engineering about it, offering a TO BE model based on AS IS for Strategic Dependency Model, Strategic Rationale Model and Business Process Model Notion.',
        githubUrl: 'https://github.com/LeonardoGaldino/iStarModels',
    },
    {
        title: 'GameBot Farmer',
        description: 'A personal project for simple purpose, collect resources in a game clicking on repeated places in specific sequences. The sequences can vary acording to user farm desires',
        githubUrl: 'https://www.github.com/mcbsf/aow_auto_clicker',
    },
    {
        title: 'Crypto Analyser',
        description: 'A academic project that analyses reddit comment sentiments about crypto currency',
        githubUrl: 'https://github.com/mcbsf/criptoanalysis',
    }
    // Add more projects here
];
